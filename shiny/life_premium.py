...
def calculate_premium(age, death_benefit, bmi, sex, loading=0.2):
    prob_death = 0.001 * (age / 30)

    # adjust probability based on BMI category
    if 40 > bmi >= 30:
...
app_ui = ui.page_sidebar(
    ui.sidebar(
        ...
        ui.input_select("height", "Height", height_options),
        ui.input_select("benefit", "Death Benefit:",
...
def server(input, output, session):
    ...
    @reactive.Calc
    def premium_value():
        return calculate_premium(input.age(), int(input.benefit()), bmi_value(), input.gender())
